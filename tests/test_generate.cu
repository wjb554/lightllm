    ...
    int P, int Hq, int Hkv, int D, int groups, float scale)
{
    ...
    int tid      = threadIdx.x;
    int dim      = tid;  // this thread handles output dimension `dim`

    extern __shared__ float smem[];
    float* q_tile = smem;                         // [B_r * D]
    float* k_tile = smem + B_r * D;               // [B_c * D]
    float* v_tile = smem + B_r * D + B_c * D;     // [B_c * D]

    // Online softmax state — one per Q token in this tile
    float m_val[B_r], l_val[B_r];
    for (int i = 0; i < B_r; i++) { m_val[i] = -1e38f; l_val[i] = 0.0f; }
    float acc[B_r] = {};  // accumulator for output dimension `dim`
    ...
    int q_tokens = min(B_r, P - q_start);  // moved here for scope
...
static Tensor prefill_attn(const Tensor& q,const Tensor& k,const Tensor& v,
    int P,int Hq,int Hkv,int D,int groups,float scale)
{
    // Threshold: when intermediate scores matrix > 32 MB, switch to tiled
...
int main(){
    ...
    // Prompt: use tokenizer to encode text
    ...
    printf("Prompt: %s → ", "The capital of France is");
    for(int id:prompt)printf("%d ",id);printf("\n");
    ...
    // Embedding: copy embed weight to CPU once (used for prefill + lm_head)
    auto ew_cpu=std::vector<float>(vocab*D);
    embed.copy_to(ew_cpu.data(),embed.nbytes());

    // Prefill embedding lookup
    {
    ...
    for(int l=0;l<n_layers;l++){
        ...
        // Adaptive GPU attention: short→3-stage, long→FlashAttention
        Tensor at=prefill_attn(q3,k3,v3,P,Hq,Hkv,HD,groups,attn_scale);
